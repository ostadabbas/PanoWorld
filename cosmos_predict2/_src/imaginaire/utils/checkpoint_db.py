...
_LOCAL_CKPT_DIR = os.environ.get(
    "COSMOS_LOCAL_CHECKPOINTS",
    os.path.join(os.path.dirname(__file__), "../../../..", "local_checkpoints"),
)
...
def get_checkpoint_path(checkpoint_uri: str) -> str:
    ...
    Falls back to local_checkpoints/ when HF download fails.
    """
    if INTERNAL:
    ...
    if checkpoint_uri.startswith("s3://"):
        try:
            ...
            ckpt_cfg = get_checkpoint_by_s3(checkpoint_uri)
            candidates = [os.path.basename(checkpoint_uri)]
            if hasattr(ckpt_cfg, "hf") and hasattr(ckpt_cfg.hf, "filename"):
                candidates.append(ckpt_cfg.hf.filename)
            if hasattr(ckpt_cfg, "name"):
                candidates.append(ckpt_cfg.name.replace("/", "_") + ".pth")
            local_dir = os.path.normpath(_LOCAL_CKPT_DIR)
            for name in candidates:
                local = os.path.join(local_dir, name)
                if os.path.exists(local):
                    log.info(f"HF download failed, using local fallback: {local}")
                    return local
            raise
