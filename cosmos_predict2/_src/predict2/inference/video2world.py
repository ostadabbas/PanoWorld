...
def read_and_process_image(img_path: str, resolution: list[int], num_video_frames: int, resize: bool = True):
    ...
    # Read the image and ensure RGB (drop alpha channel from RGBA PNGs, etc.)
...
def read_and_process_video(
    ...
    if num_latent_conditional_frames < 1:
        raise ValueError(f"num_latent_conditional_frames must be >= 1, but got {num_latent_conditional_frames}")
...
class Video2WorldInference:
    ...
    def generate_vid2world(
        ...
        self._last_latent = sample.detach().clone().cpu()
