...
def get_negative_emb():
    ...
    # NOTE: this is the canonical WAN T2V negative prompt. Users load the precomputed
    # embedding `umT5_wan_negative_emb.pt` from S3 instead of re-running this helper.
    # The exact wording originally used by WAN to derive the embedding is intentionally
    # preserved in English form below; if you need bit-exact reproduction with WAN you
    # would need to re-encode with the original Chinese prompt instead.
    neg_prompt = (
        "vivid tones, overexposed, static, blurry details, subtitles, style, artwork, "
        "painting, frame, still, grey overall, worst quality, low quality, JPEG "
        "compression artifacts, ugly, mutilated, extra fingers, poorly drawn hands, "
        "poorly drawn face, deformed, disfigured, malformed limbs, fused fingers, "
        "still frame, cluttered background, three legs, crowded background people, "
        "walking backwards"
    )
