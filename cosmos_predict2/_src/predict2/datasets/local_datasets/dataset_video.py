...
class WeightedDistributedSampler(Sampler):
    """Distributed weighted sampler for ``ConcatDataset``-style mixtures.

    Builds a per-clip weight vector from per-subset target proportions and
    draws samples with replacement using ``torch.multinomial``. Sharding
    across ranks is identical to ``DistributedSampler`` (stride-based).

    Args:
        dataset: Dataset whose total length determines the sampling pool.
            Expected to expose ``cumulative_sizes`` (e.g. ``ConcatDataset``)
            so we can map subset → index range.
        subset_weights: Tuple of target proportions per subset, in the same
            order as ``dataset.datasets``. Sum is normalized.
        num_samples_per_replica: Number of samples this rank emits per epoch.
            Defaults to ``len(dataset)`` divided across replicas.
        seed: Base seed; combined with the per-epoch counter via ``set_epoch``.
    """

    def __init__(
        self,
        dataset,
        subset_weights: tuple[float, ...] | list[float],
        num_samples_per_replica: Optional[int] = None,
        seed: int = 0,
    ) -> None:
        if not hasattr(dataset, "cumulative_sizes"):
            raise ValueError(
                "WeightedDistributedSampler expects a ConcatDataset-like object "
                "exposing `cumulative_sizes`."
            )
        cum = list(dataset.cumulative_sizes)
        sizes = [cum[0]] + [cum[i] - cum[i - 1] for i in range(1, len(cum))]
        if len(sizes) != len(subset_weights):
            raise ValueError(
                f"subset_weights length {len(subset_weights)} != number of "
                f"subsets {len(sizes)}"
            )
        total_w = float(sum(subset_weights))
        norm_weights = [float(w) / total_w for w in subset_weights]
        per_clip = []
        for w_subset, sz in zip(norm_weights, sizes):
            per_clip.extend([w_subset / sz] * sz)
        self.weights = torch.tensor(per_clip, dtype=torch.float)

        self.num_replicas = parallel_state.get_data_parallel_world_size()
        self.rank = parallel_state.get_data_parallel_rank()
        total = num_samples_per_replica or (len(dataset) // self.num_replicas)
        self.num_samples = total
        self.seed = seed
        self.epoch = 0

        log.info(
            f"WeightedDistributedSampler: subsets={sizes}, weights={norm_weights}, "
            f"num_samples_per_replica={self.num_samples}, replicas={self.num_replicas}"
        )
    ...
    def __iter__(self):
        ...
        # Sample globally then stride to this rank for balanced sharding.
        n_global = self.num_samples * self.num_replicas
        idx = torch.multinomial(self.weights, n_global, replacement=True, generator=g)
        idx = idx[self.rank : n_global : self.num_replicas]
...
def get_weighted_concat_sampler(dataset, subset_weights, num_samples_per_replica=None, seed=0):
    """Hydra-friendly factory for ``WeightedDistributedSampler``."""
