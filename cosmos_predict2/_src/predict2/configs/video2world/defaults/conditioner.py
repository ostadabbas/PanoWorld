...
@dataclass(frozen=True)
class PanoCondition(Video2WorldCondition):
    """Extends Video2WorldCondition with CLIP image embeddings and reference latent
    for panoramic generation (ARGUS-style latent concatenation).

    Fields:
      - img_context_emb: CLIP visual features (B, 257, 1280) for I2VCrossAttention.
      - reference_latent_B_C_T_H_W: VAE-encoded masked equirectangular video
        (B, 16, T_lat, H_lat, W_lat) concatenated as extra input channels to the DiT.
    """
