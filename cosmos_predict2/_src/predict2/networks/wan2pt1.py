...
class VideoRopePosition3DEmb(VideoPositionEmb):
    ...
    def _get_equirectangular_grid(self, H: int, W: int, device: torch.device):
        """Generate equirectangular-aware position grids.

        Height (latitude): sin() mapping compresses polar regions where
        equirectangular projection over-represents area. sin() is monotonic
        on [-pi/2, pi/2] so positions remain well-ordered.

        Width (longitude): linear mapping. sin() is NOT monotonic on [-pi, pi]
        — it maps both edges AND center to the same position value, causing
        the model to treat them as identical and producing double-wrapping
        artifacts with vertical stripe distortions.
        """
        grid_h = torch.linspace(-math.pi / 2, math.pi / 2, H, device=device, dtype=torch.float32)
        grid_h = (torch.sin(grid_h) + 1) / 2 * (H - 1)
    ...
    def generate_embeddings(
        ...
        Returns:
            Tensor: RoPE embeddings of shape ((T*H*W), 1, 1, D).
        """
        ...
        if self.grid_type == "equirectangular":
            grid_h, grid_w = self._get_equirectangular_grid(H, W, self.seq.device)
            freqs_h = torch.outer(grid_h, h_spatial_freqs)
            freqs_w = torch.outer(grid_w, w_spatial_freqs)
        else:
