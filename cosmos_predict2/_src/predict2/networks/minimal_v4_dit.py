...
class I2VCrossAttention(Attention):
    ...
    def compute_attention(self, q, k, v, k_img, v_img):
        ...
        if k_img is not None and v_img is not None:
            result = result + self.attn_op(q, k_img, v_img)
        return self.output_dropout(self.output_proj(result))
...
class VideoRopePosition3DEmb(VideoPositionEmb):
    ...
    def generate_embeddings(
        ...
        if self.grid_type == "equirectangular":
            grid_h = torch.linspace(-math.pi / 2, math.pi / 2, H, device=self.seq.device, dtype=torch.float32)
            grid_h = (torch.sin(grid_h) + 1) / 2 * (H - 1)
            grid_w = self.seq[:W]
            half_emb_h = torch.outer(grid_h, h_spatial_freqs)
            half_emb_w = torch.outer(grid_w, w_spatial_freqs)
        else:
...
class MiniTrainDIT(WeightTrainingStat):
    ...
    def forward(
        ...
        if img_context_emb is not None:
            ...
            img_context_emb = self.img_context_proj(img_context_emb.to(dtype=self.img_context_proj[0].weight.dtype))
